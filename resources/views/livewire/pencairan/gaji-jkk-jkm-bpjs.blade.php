<div class="flex flex-col gap-3">
    <x-main.page-header
        title="Formulir Permohonan Pengajuan Pencairan - Gaji / JKK / JKM / BPJS | {{ isset($editData) ? 'Ubah Data' : 'Tambah Data' }}">
    ...
    <div class="card border border-slate-300 bg-base-100 w-full">
        <div class="card-body p-0 gap-0">
                    ...
                    <div class="w-full grid grid-cols-12 px-4 gap-3">
                        ...
                        <div class="col-span-12 md:col-span-12 lg:col-span-3">
                            <label for="jenis_pembayaran"
                                class="block text-sm font-medium mb-2 {{ $errors->has('state.jenis_pembayaran') ? 'text-red-500' : '' }}">
                                Jenis Pembayaran :
                            </label>
                            <div class="relative">
                                <select wire:model="state.jenis_pembayaran" id="jenis_pembayaran"
                                    name="jenis_pembayaran"
                                    class="w-full select @error('state.jenis_pembayaran') select-error @enderror"
                                    aria-describedby="jenis_pembayaran-helper"
                                    placeholder="Masukan Jenis Pembayaran..." required autocomplete="false">
                                    <option disabled>Pilih Jenis Pembayaran</option>
                                    @foreach ($staticData['jenis_pembayaran'] as $item)
                                        <option value="{{ $item }}">{{ $item }}</option>
                                    @endforeach
                                </select>
                                <div
                                    class="absolute inset-y-0 end-6 {{ $errors->has('state.jenis_pembayaran') ? 'flex' : 'hidden' }} items-center pointer-events-none pe-3">
                                    <svg class="shrink-0 size-4 text-red-500" xmlns="http://www.w3.org/2000/svg"
                                        width="24" height="24" viewBox="0 0 24 24" fill="none"
                                        stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                        stroke-linejoin="round">
                            ...
                            @error('state.jenis_pembayaran')
                                <p class="text-xs text-red-600 mt-1" id="jenis_pembayaran-helper">
                                    {{ $message }}
                                </p>
                            @enderror
                        </div>
            ...
            <div class="card-actions text-xs font-semibold text-slate-600 bg-slate-200 rounded-b-lg px-5 py-2">
                Formulir Permohonan Pengajuan Pencairan - Gaji / JKK / JKM / BPJS |
